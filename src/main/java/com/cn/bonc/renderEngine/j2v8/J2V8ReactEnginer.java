package com.cn.bonc.renderEngine.j2v8;
...
public class J2V8ReactEnginer {
    ...
    private void initRuntime(V8 runtime, List<V8Value> runtimeObject, RenderingContext routerCtx) {
        String contextPath = routerCtx.getContext().getContextPath() != ""? routerCtx.getContext().getContextPath().substring(1): "";

        try {
            executeScript(runtime, runtimeObject, getJSResource("static/js/j2v8-polyfill.js"));
            runtime.executeVoidScript("global.__CONTEXT_PATH__ = '" + contextPath + "'");
            executeScript(runtime, runtimeObject, getJSResource(getJsResourcePath("static/asset-manifest.json")));
            executeScript(runtime, runtimeObject, getJSResource("static/js/render.js"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public String getJSResource(String path) throws IOException{
    ...
    public String getJsResourcePath(String path) throws IOException{
        String assetMainifest = getJSResource(path);
